<?php
	...
	class Actions extends \caspar\core\Actions
	{
		...
		protected function _sendGameInvite(GameInvite $invite, User $user, User $player)
		{
			...
			$plain_content = str_replace(array('%username%', '%playername%', '%game_id%'), array($user->getUsername(), $player->getUsername(), $invite->getGame()->getId()), file_get_contents(CASPAR_MODULES_PATH . 'main' . DS . 'templates' . DS . 'game_invite.txt'));
