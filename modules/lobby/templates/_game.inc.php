<li id="game_<?php echo $game->getId(); ?>_list">
	<div class="tooltip">
		...
		<div class="game_tooltip_turn">
			<span id="game_<?php echo $game->getId(); ?>_player_turn"<?php if (!$game->isInvitationConfirmed() || !$game->isPlayerTurn() || !$game->isUserPlayerReady()): ?> style="display: none;"<?php endif; ?>>It is <?php echo ($game->getPlayer()->getId() == $csp_user->getId()) ? 'your' : $game->getPlayer()->getCharactername()."'s"; ?> turn</span>
			<span id="game_<?php echo $game->getId(); ?>_opponent_turn"<?php if (!$game->isInvitationConfirmed() || !$game->isOpponentTurn() || !$game->isUserPlayerReady()): ?> style="display: none;"<?php endif; ?>>It is <?php echo ($game->getOpponent()->getId() == $csp_user->getId()) ? 'your' : $game->getOpponent()->getCharactername()."'s"; ?> turn</span>
		...
		<div class="game_tooltip_info" id="game_<?php echo $game->getId(); ?>_pickandplace" style="<?php if ($game->isUserPlayerReady() || !$game->isInvitationConfirmed()) echo 'display: none;'; ?>">Pick and place cards</div>
		<div class="game_tooltip_info" id="game_<?php echo $game->getId(); ?>_info" style="<?php if (!$game->isUserPlayerReady() || !$game->isInvitationConfirmed()) echo 'display: none;'; ?>">
			<span id="game_<?php echo $game->getId(); ?>_phase_pre" style="<?php if ($game->isUserPlayerReady()) echo 'display: none;'; ?>">Pick and place cards</span>
	...
	<button class="ui_button button-cancel" onclick="Devo.Play.cancelGameInvitation(<?php echo $game->getId(); ?>);return false;" style="<?php if ($game->isInvitationConfirmed()) echo 'display: none;'; ?>"><img src="/images/spinning_16.gif" style="display: none;">Cancel</button>
