		...
		public function getLinesByRoomId($room_id, $since = null, $limit = null)
		{
			...
			if ($res = $this->doSelect($crit)) {
				while ($row = $res->getNextRow()) {
					...
					array_unshift($lines, array('line_id' => $row->get('chat_lines.id'), 'user_id' => $user_id, 'user' => array('username' => ($user_id) ? $row->get('users.username') : 'System', 'charactername' => ($user_id) ? $row->get('users.charactername') : 'System', 'race' => ($user_id && $row->get('users.race')) ? \application\entities\User::getRaceNameByRace($row->get('users.race')) : '', 'level' => ($user_id) ? $row->get('users.level') : 0, 'friends' => ($user_id) ? \caspar\core\Caspar::getUser()->isFriends($user_id) : false), 'text' => htmlspecialchars($row->get('chat_lines.text'), ENT_NOQUOTES, 'utf-8'), 'posted' => $posted, 'posted_formatted_hours' => date('H:i', $posted), 'posted_formatted_date' => date('d/m/Y', $posted)));
				}
			}
